...
//! and each reports its best round: noise only ever slows a run down, so the fastest round is the
//! closest to the undisturbed cost. Running one to the end and then the next would charge every
