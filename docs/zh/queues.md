...
队列数的是失败的投递 - 消费者没结算就消失的那一次 - 仲裁队列把这个计数盖在每一次重新投递的
`x-delivery-count` 上。有它的地方，上限读的就是它。处理器把自己的消息要回去，并不以这种方式花掉
一次尝试：RabbitMQ 4.3 不数重新入队，而 4.2 会数，所以由处理器挑起的重试循环，是靠框架自己的重试
计数消息头来终结的，运行时在它发布的每一个副本上把它加一。经典队列则什么也不数，那里全部的计数
就是这个消息头。
...
上限够不到这条路。等待队列是把消息重新发布，而不是把它退回，于是每一个延迟副本上队列自己的计数
都从头开始，尝试次数也不会被带下去：在带 `.delay(..)` 的队列上一直回答 `retry_after` 的处理器，
会一直转到它自己停下为止。需要上限生效的地方，把延迟交给运行时：它发布的副本会把计数放在消息头
里带着走。
