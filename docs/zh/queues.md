...
描述符点名处理器消费的队列，并写出该队列应有的样子：持久化、交换机绑定、预取，以及原始的
`x-*` 参数。`RabbitQueue` 就是经典队列；描述符直接写在 `#[subscriber(..)]` 属性里：
...
RabbitMQ 在声明时通过 `x-queue-type` 选定队列的实现，而已存在的队列的类型再也改不了。所以实现
就是描述符本身，而不是它上面的一个设置：`RabbitQueue` 是经典的单节点队列，`RabbitQuorumQueue`
是用 Raft 复制的那一种。两者都按自己点名的类型声明。

它们描述的东西是一样的 - 绑定、预取、死信参数、原始 `x-*`、`.delay(..)`，差别在重试，见
[限制重试次数](#capping-the-retries)。

仲裁队列按定义就是持久化、非独占、不自动删除的，所以它上面没有 `.durable(..)`、`.exclusive(..)`
和 `.auto_delete(..)`：与自身类型矛盾的队列根本写不出来，而不是等服务启动时被拒绝。

RabbitMQ 4 默认拒绝瞬态（非持久化）的非独占队列：在经典队列上，除非队列写了 `.exclusive(true)`，
否则保持默认的持久化。
...
`.dead_letter_exchange("dlx")`（需要时再加 `.dead_letter_routing_key(..)`）在两个描述符上都设定
队列原生的死信目标。丢弃消息的处理器按 `basic.reject(requeue = false)` 结算，消息就被路由到那里。这是队列自己的
拓扑，对任何一次拒绝都生效，无论拒绝来自谁。

## 限制重试次数 { #capping-the-retries }

一直要求再投递一次的处理器，会让消息一直转下去，直到有人来干预。`include` 之后的两个步骤终结
它：
...
`max_attempts(n)` 是一条消息能得到多少次投递，含第一次；`dead_letter(name)` 是它们用尽之后消息
去往何处，按它到达时的原样重新发布。这里的目的地是默认交换机上的路由键，所以这个名字就是耗尽的
投递最终落入的队列。

在服务自己声明的仲裁队列上，这一对会变成队列自己的拓扑：队列带着 `x-delivery-limit` 和指向该
名字的死信路由被声明，由服务器来执行。该参数比上限小一：服务器数的是消息经受住的退回次数，而
上限数的是它得到的投递次数。消费者一次次死掉的那条消息会自己离开队列，此时并没有服务在替它
计数。也正因如此，`out_retry(policy)` 在仲裁队列上不通过编译：本进程根本不发布任何副本，也就
没有发布者可点名。

两个步骤要么都写，要么都不写。有上限却没地方送走耗尽的投递，只会把它丢掉，所以订阅会拒绝只写
一半的声明，并点出缺的那一步。

服务不声明的仲裁队列，带的是声明它的人给的参数，所以那里订阅同样拒绝这一对，而不是许下一个
无人执行的上限：在 Broker 上配好 `x-delivery-limit` 和 `x-dead-letter-exchange`，处理器就照常
挂载。队列的计数 crate 两种情况下都读得到。队列属于服务、而重试又不是某一个处理器的事时，描述符
自己写出同样的策略：
...
经典队列什么也不数，也没有自己的投递上限，所以那里的上限属于运行时：它在自己发布的副本的
`x-ruststream-retry-count` 消息头里记尝试次数，副本按队列自己的名字回去 - 在默认交换机上正是这个
名字寻址到该队列。副本经由每个注册都已经有的发布者发出，也就是 Broker 的默认发布策略。那个不
合用时，点名另一个：
...
`redelivered` 标志只说明消息之前被看见过，它的每一次投递都报不出计数。
...
副本在服务进程里等待，整个窗口内是至多一次，并按队列自己的名字发回去。

`.delay(..)` 把延迟挪到 Broker 上：消息停在等待队列里，带着自己的 TTL，TTL 到期就以死信的方式
回到原队列，于是窗口中途重启也不丢东西。服务在这条路上不发布自己的副本。

```rust
--8<-- "crates/ruststream-lapin/examples/lapin_topology.rs:delay"
```

等待队列放出来的副本是一条新消息，而新消息服务器从零开始数：仲裁队列自己的计数器在它身上从头
开始。于是副本改为带上框架的计数 - 就是运行时在自己发布的副本上
写的那个 `x-ruststream-retry-count` - 每回来一次就加一。等核心在这条路上读这个计数，上限在带延迟
的队列上也就生效了：那是 0.7.0-rc.5 之后的那个发布。在这个 crate 的下界点到它之前，在带
`.delay(..)` 的队列上一直回答 `retry_after` 的处理器，会一直转到它自己停下为止。
