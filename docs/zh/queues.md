...
目标。丢弃消息的处理器按 `basic.reject(requeue = false)` 结算，消息就被路由到那里。这是队列自己的
...
在服务自己声明的仲裁队列上，这一对会变成拓扑：队列带着 `x-delivery-limit` 和指向该名字的死信
路由被声明，于是消费者一次次死掉的那条消息会自己离开队列，此时并没有服务在替它计数。该参数比
上限小一：服务器数的是消息经受住的退回次数，而上限数的是它得到的投递次数。经典队列没有自己的
投递上限，于是这份声明由运行时在重试路径上执行 - 同样的承诺，只是计数留在服务里。

服务器保有两个计数器，上限读其中较大的那个。仲裁队列把 `x-delivery-count` 盖在失败的投递上 -
消费者没结算就消失的那一次。`x-death` 表数的则是服务器自己带着消息走过的圈：它离开的每个队列
一条记录，而这个 crate 把会把消息带回来的那两种原因加在一起，`rejected` 与 `expired`。处理器把
自己的消息要回去，在这两处都不花掉一次尝试：RabbitMQ 4.3 不数重新入队，而 4.2 会数，所以由处理器
挑起的重试循环，是靠框架自己的重试计数消息头来终结的，运行时在它发布的每一个副本上把它加一。
没有死信路由的经典队列则什么也不数，那里全部的计数就是这个消息头。
...
它寻址到该队列；它经由每个注册都已经有的发布者离开，也就是 Broker 默认的发布策略。这个不合用
时，点名另一个：
...
这个位置接受槽位的各个步骤：其后的 `.codec(..)` 与 `.transform(..)`，以及副本该去别处而非它来
处那个队列时的 `.to(name)`。这里的变换读取正在重试的那次投递，就像应答的变换读取请求那样。
...
副本对服务器来说是一条新消息，服务器从零开始数它：等待队列在每一圈都重新写自己的 `x-death`
记录，仲裁队列自己的计数器也从头开始。于是副本改为带上框架的计数 - 就是运行时在自己发布的副本上
