...
路由被声明，服务器自己就把耗尽的投递带走，即便此刻没有服务在运行。该参数比上限小一：服务器数的
是消息经受住的退回次数，而上限数的是它得到的投递次数。经典队列没有自己的投递上限，于是这份声明
由运行时在重试路径上执行 - 同样的承诺，只是计数留在服务里。

计数本身属于队列，前提是这种队列在计数。仲裁队列会给每一次重新投递盖上 `x-delivery-count`，上限
读的就是它；经典队列什么也不数，于是由框架自己的重试计数消息头把尝试次数带下去。
