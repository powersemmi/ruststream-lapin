...
(() => {
  ...
  const ENVIRONMENT = [
    ...
    ["os", ["os"]],
    ["broker", ["broker"]],
    // What a request the client waits for cost on this transport when the run was taken. The
    // `broker-bound` mark in the table above is arithmetic over it, so it is shown next to it.
  ...
  function scenarios(results, labels, lang) {
    ...
    for (const scenario of results.scenarios) {
      ...
      // The scenario, then the three runs in the order they cost, then what each of the two
      // measured runs adds to the raw one. `broker-bound` is a property of the whole row, so it
      // rides the name rather than being repeated in both overhead columns.
